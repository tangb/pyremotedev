...
logging.basicConfig(level=logging.INFO, format=u'%(asctime)s %(levelname)s [%(name)s]: %(message)s')

APP_NAME = u'pyremotedev'
APP_AUTHOR = u'tangb'
...
def reset_logging(level, to_file=None):
    ...
    if to_file:
        logging.basicConfig(filename=to_file, level=level, format=u'%(asctime)s %(levelname)s [%(name)s]: %(message)s')
    else:
        logging.basicConfig(level=level, format=u'%(asctime)s %(levelname)s [%(name)s]: %(message)s')
...
if params[u'master']:
    ...
    try:
        master = pyremotedev.PyRemoteDevMaster(profile)
        master.start()
    ...
    try:
        slave = pyremotedev.PyRemoteDevSlave(profile)
        slave.start()
        while True:
            time.sleep(1.0)
